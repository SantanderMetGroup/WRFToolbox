...
!  March 2012 - M. Garcia-Diez (Santander Meteorolgy Group, Univ. Cantabria) 
!
!=================================Make Executable============================
!  Make executable:
!
...
! GMS.UC: Markel March 2012 
	...
	path_to_output = TRIM(path_to_output)//"/"
      ENDIF
      input_file = TRIM(path_to_input)//TRIM(input_name)
      output_file = TRIM(input_file)//"_PLEV"
!
! GMS.UC: Lluis Dec.09
!
      !
      ...
      ! Markel: I don't understand why is this useful, I don't
      ! see any loop to process all the input files. 
      !
      ! Do we have a list of the fields that we want in output?
      ...
      PRINT *, "RUNNING p_interp V1.0 on ",input_file
...
!        interp_levels(num_metgrid_levels) = interp_levels(num_metgrid_levels) * 100.0   !!! Pa
      ...
      ! Do we have a list of field that we want on output?
        ...
        !
         ...
         enddo
         !
         ! GMS.UC: Lluis Dec.09 Combine PH and PHB to get the geopotential 
         ! and destagger it. It is stored in the array "ght"
         !
         PRINT *,'  PH, PHB...'
       	 ivar1=-1
         ...
         ENDIF
         !
         ! GMS.UC: Lluis Dec.09 Read topography and store it in array "ter"
         !
         PRINT *,'  HGT...'
       	 ivar1=-1
...
! TRAIN FILE 
        ...
        loop_variables : DO ivar = 1, nvars
          ...
          fix_meta_stag = .FALSE.
          rcode = nf_redef(mcid)
          PRINT *,'jvar3: ',jvar, dnamei(ishape(1:idm))
	  PRINT *,' dnamej: ',dnamej
          DO ii = 1, idm
            unstagvar = .FALSE. 
...
!! GMS.UC: Lluis. Jan 10 Correction in assignation of new dimensions (new dims) id
	  ...
	  PRINT *,'var def: ',mcid
	  PRINT *,'var defA: ',cval
	  PRINT *,'var defB: ', itype
	  PRINT *,'var defC: ', idm
	  PRINT *,'var defD: ', jshape
	  PRINT *,'var defE: ', jvar
...
! GET THE DIMS FOR INPUT AND OUTPUT FROM THE SHAPE
          ...
          ENDDO
          IF (debug) THEN
            write(6,*) 'VAR: ',trim(cval),' jvar:',jvar
...
! READ THE DATA FROM INPUT FILE
	  ...
	  ELSEIF (itype == 5) THEN          ! real
        ...
        write(6,*) ' ******  DIMS OUT: ',dims_out   !!! borrar
...
! GMS.UC:Lluis Dec.09
        ...
        ENDIF
! jf20120102 (begin)
        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'PBLHGT') /= 0 ) THEN
          !
          !   Unified PBL height following Nielsen-Gammon et al. (2008) 
          ! 
          section = "'PBLHGT'"
          ...
          jvar = jvar + 1
          varnameIN='PBLHGT'
          longdescIN='PBL height (1.5-theta-increase method)'
          unitsIN='m'
          CALL def_var (mcid, jvar, varnameIN, 5, 3, jshape2d, "XY", longdescIN, unitsIN, "-"   &
            , "XLONG XLAT", output_netCDF4)
          IF (ALLOCATED(data3)) DEALLOCATE(data3)
          ALLOCATE (data3(dims2d_out(1), dims2d_out(2), dims2d_out(3), 1))
          CALL pbl_height(ncid, debug, dims_in(1), dims_in(2), dims_in(3), dims_in(4), ght, ter, data3)
          IF (debug) THEN
            PRINT *,'VAR: ',varnameIN,'idvar:',jvar
...
SUBROUTINE massvertint(ncid, variables, Nvar, dbg, nx, ny, nz, nt, integral)
  ...
  INTEGER, INTENT(IN)                            :: nx, ny, nz, nt
  LOGICAL, INTENT(IN)                            :: dbg
    ! jf: La matriz que se pasa en el programa principal por este argumento
    !     (data3) tiene rango 4!! y aqui se recibe en una de rango 3.
    ! /!\ Revisar!
...
! jf20120102 (begin)
SUBROUTINE pbl_height(ncid, dbg, nx, ny, nz, nt, ght, ter, rval)
  !
  ...
  REAL, DIMENSION(nx,ny,nt), INTENT(OUT) :: rval
  INTEGER                                :: id_T, ix,iy,iz,it, rcode
  REAL, DIMENSION(nx,ny,nz,nt)           :: dataval
  ...
  IF (dbg) PRINT *,'PBL height computation...'

  rcode = nf_inq_varid(ncid, 'T', id_T)
  IF (rcode /= nf_noerr) PRINT *,nf_strerror(rcode)
  rcode = nf_get_var_real(ncid, id_T, dataval)
  IF (rcode /= nf_noerr) PRINT *,nf_strerror(rcode)
  minlevel = minloc(dataval, 3)
  threshold = minval(dataval, 3) + 1.5
  DO it=1,nt
    DO ix=1,nx
      DO iy=1,ny
        DO iz=minlevel(ix,iy,it),nz
          if (dataval(ix,iy,iz,it).ge.threshold(ix,iy,it)) EXIT ! iz contains the level
        END DO
        ! Interpolate between levels iz-1 and iz
        rval(ix,iy,it) = ght(ix,iy,iz,it)+ &
                            (threshold(ix,iy,it)-dataval(ix,iy,iz,it)) * &
                               (ght(ix,iy,iz,it)-ght(ix,iy,iz-1,it)) / &
                               (dataval(ix,iy,iz,it)-dataval(ix,iy,iz-1,it))
        ! Convert to meters and remove terrain height
        rval(ix,iy,it) = rval(ix,iy,it)/9.81 - ter(ix,iy)
      END DO
    END DO
  END DO
...
SUBROUTINE handle_err(rcode)
    INTEGER rcode
    print *,"Error code:", nf_strerror(rcode)
    stop
END SUBROUTINE
!---------------------------------------------------------------------
