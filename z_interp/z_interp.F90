...
!  The program reads namelist.zinterp
!  January 2011 - Lluis Fita UC
!  September 2012 - M. Garcia-Diez (Santander Meteorolgy Group, Univ. Cantabria)
!  from an adaptation of 'p_interp' of Cindy Bruyere
!  
!
!=================================Make Executable============================
!  Make executable:
...
!
!  GMS.trueno:
!      gfortran z_interp.F90 -L//home/lluis/bin/netcdf-4.0.1/lib -lnetcdf \
!      -I/home/lluis/bin/netcdf-4.0.1/include -o z_interp
!  GMS.UC.new:
!      gfortran z_interp.F90 -L/software/CentOS/5.2/netcdf/4.1.1/gcc-gfortran4.1.2/lib -lnetcdf \
!      -I/software/CentOS/5.2/netcdf/4.1.1/gcc-gfortran4.1.2/include -o z_interp
!
!  GMS.UC.old:
!      pgf90 z_interp.F90 -L/software/ScientificLinux/4.6/netcdf/3.6.3/pgf716_gcc/lib -lnetcdf -lm \
!      -I/software/ScientificLinux/4.6/netcdf/3.6.3/pgf716_gcc/include -Mfree -o z_interp
!
!  DEC Alpha
!      f90 z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf/include  -free  -o z_interp
!
!   Linux flags
!      pgf90 z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf/include  -Mfree  -o z_interp
!
!   Sun flags
!      f90 z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf/include  -free  -o z_interp
...
!   SGI flags
!      f90 z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf/include  -freeform  -o z_interp
!
!   IBM flags 
!      xlf z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf/include  -qfree=f90  -o z_interp
!
!   Mac flags (with xlf compiler)
!      xlf z_interp.F90 -L/usr/local/netcdf-xlf/lib -lnetcdf -lm  \
!      -I/usr/local/netcdf-xlf/include  -qfree=f90  -o z_interp
!
!   Mac flags (with g95 compiler)
!	g95 z_interp.F90 -L/usr/local/netcdf/lib -lnetcdf -lm  \
!	-I/usr/local/netcdf/include -ffree-form -o z_interp
!
...
!============================================================================
      ...
      REAL,              ALLOCATABLE, DIMENSION(:,:,:,:) :: phb, qv, tk, rh
      ...
      REAL, DIMENSION(99)                                :: interp_levels, interz_levels
      REAL                                               :: rval
      REAL                                               :: MISSING=1.e36
      INTEGER                                            :: LINLOG = 1
...
! GMS.UC: Markel March 2012 
      ...
      NAMELIST /interp_in/ interz_levels, interp_method, extrapolate, unstagger_grid, p_top
      ...
      ! Read parameters from Fortran namelist
        DO funit=10,100
           ...
           IF (.not. is_used) EXIT
        END DO
        OPEN(funit,file='namelist.zinterp',status='old',form='formatted',iostat=ios)
        IF ( ios /= 0 ) STOP "ERROR opening namelist.zinterp"
        READ(funit,io)
...
!        WRITE ( command , FMT='("ls -1 ",A300," > .foo")' ) TRIM ( inputlong_name )
        ...
        DO loop = 1 , number_of_input_files
           ...
           IF ( output_name == ' ' ) THEN
              ...
              output_file_names(loop) = TRIM(output_file_names(loop))//"_ZLEV"
           ELSE
      ...
      write(6,*) 
      write(6,*) "##############################################################"
      write(6,'(A,i4,A)') " RUNNING z_interp V0.1 on ", number_of_input_files, " file(s)."
      ...
      write(6,*)
      write(6,*) "INTERPOLATING TO HEIGHT LEVELS: "
      num_height_levels = 0
      DO WHILE (interz_levels(num_height_levels+1) /= -99999.) 
!        IF (interz_levels(num_height_levels+1) == -99999.) EXIT
        num_height_levels = num_height_levels + 1
        PRINT *,'level:',num_height_levels,':',interz_levels(num_height_levels),' m'
!        if (mod(num_height_levels,8) /= 0 )write(6,'(f8.3,$)') interz_levels(num_height_levels)
!        if (mod(num_height_levels,8) == 0 )write(6,'(f8.3)') interz_levels(num_height_levels)
!        interz_levels(num_height_levels) = interz_levels(num_height_levels) * 100.0   !!! Pa
      END DO
!"!      interp_levels = interp_levels*100.0  !!! Pa
      write(6,*)
      write(6,*)
      PRINT *,'num_height_levels: ',num_height_levels
...
! READ ALL DIMS FROM INPUT FILE AND CREATE SOME DIMS FOR OUTPUT FILE
        ...
        j = j + 1
        dnamej(j) = 'num_height_levels'
        dvalj(j) = num_height_levels
...
! DEALING WITH THE GLOBAL ATTRIBUTES
	  ...
	  elseif ( itype .eq. 4 ) then     ! integers
	    rcode = nf_get_att_int (ncid, nf_global, cname, ival)
	       ...
	       IF (cname .eq. 'BOTTOM-TOP_GRID_DIMENSION') ival = num_height_levels
               IF (debug) &
	         write(6,'("     i = ",i2," : ",A," = ",i7)') &
                       i,cname,ival        
...
!#!         IF (ivar1 == 1 ) THEN
!#!           PRINT *,"Extracting 'P_TOP' from 'namelist.zinterp'"
!#!           data1=p_top
!#!         ELSE
...
!#!                   (data1(1,1,1,1)-interp_levels(num_height_levels)) > 0.0 ) THEN
!#!                 write(6,*)
!#!                 write(6,*) "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
...
!           rcode = nf_close ( ncgid )
           ...
           CALL extract_from_geogrid(debug, geofilename, varnames, 1, iweg-1, isng-1, ibtg-1, datageogrid) 
...
!
	...
	END DO
        ...
        allocate (pres_out(iweg-1, isng-1, num_height_levels, times_in_file))
        do i = 1, num_height_levels
          pres_out (:,:,i,:) = interz_levels(i)
        enddo
        ...
        loop_variables : DO ivar = 1, nvars
          ...
          DO ii = 1, idm
            ...
            IF ( test_dim_name == 'bottom_top' .OR. test_dim_name == 'bottom_top_stag' ) THEN
                 ...
                 test_dim_name = 'num_height_levels'
                 interpolate = .TRUE.
                 unstagvar = .TRUE. 
            ENDIF
	    ...
	    rcode = nf_get_var_real(ncid, ivar, data1)
            ...
            IF (idm >= 4 .AND. interpolate) THEN  
               ...
               IF ( dims_in(1) == iweg .AND. .not. unstagger_grid ) THEN
                  CALL interz (debug, data2, data1, height_stagU, interz_levels, psfc, ter, tk, qv,   &
                                 iweg, isng-1, ibtg-1, dims_in(4), &
                                 num_height_levels, LINLOG, extrapolate, .FALSE., MISSING)
               ELSEIF ( dims_in(2) == isng .AND. .not. unstagger_grid ) THEN
                  CALL interz (debug, data2, data1, height_stagV, interz_levels, psfc, ter, tk, qv,   &
               ...
               ELSEIF ( dims_in(1) == iweg .AND. unstagger_grid ) THEN
                  ...
                  CALL interz (debug, data2, data3, height_field, interz_levels, psfc, ter, tk, qv,   &
               ...
               ELSEIF ( dims_in(3) == ibtg ) THEN
                  ...
                  deallocate(data3)
               ELSE
                  CALL interz (debug, data2, data1, height_field, interz_levels, psfc, ter, tk, qv,   &
...
!!! We have some special variables we are interested in: PRES, TT, GHT, MSLP
        ...
        DO ii = 1, 4
          ...
          IF ( ii == 3 ) test_dim_name = 'num_height_levels'
...
! GMS.UC:Lluis Dec.09
! Allways write vector with vertical interpolated height levels
!        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'HGT') /= 0 ) THEN
          !!! HEIGHT: Height levels vector 
           jvar = jvar + 1
           jshape1d=RESHAPE((/jshape(3),jshape(2),jshape(1)/),(/3/))
           dims1d_out=1
           dims1d_out(3)=dims_out(3)
           varnameIN='HEIGHT'
           longdescIN='height levels above surface'
           unitsIN='m'
           CALL def_var (mcid, jvar, varnameIN, 5, 1, jshape1d, "Z", longdescIN, unitsIN, "-", &
             "XLONG XLAT", output_netCDF4)
           IF (debug) THEN
             write(6,*) 'VAR: HGT idvar:',jvar
             write(6,*) '     DIMS OUT: ',dims1d_out
           ENDIF
           rcode = nf_put_vara_real (mcid, jvar, 1, num_height_levels, interz_levels)
           IF (debug) write(6,*) '     SAMPLE VALUE OUT = ',pres_out(1,1,num_height_levels/2,1)
!         END IF
        ...
        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'MSLP') /= 0 ) THEN
          ...
          !!! MSLP 
           ...
           data2=0.
           CALL mean_sealevelpress(debug, data2, pres_field, interp_levels, psfc, ter, tk, qv,       &
             iweg-1, isng-1, ibtg-1, dims_in(4), dims_out(4),                                 &          
                          num_height_levels, LINLOG, extrapolate, MISSING)
         ...
         !!!! Filtered MSLP
           ...
           CALL spatialfiltering(debug, data2, grid_filt, ntimes_filt, dims_out(1), dims_out(2),       &
             dims_out(4), 1, data3)
           IF (debug) THEN
        ...
        !!!! Accumulated total precipitation 
          ...
          CALL variablessum(debug, ncid, varnames, 2, dims_out(1), dims_out(2), dims_out(4), 1, data3) 
          IF (debug) THEN
       ...
       IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'PRES') /= 0 ) THEN
        ...
        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'TT') /= 0 ) THEN
           ...
           allocate (data2(dims_out(1),dims_out(2),dims_out(3),dims_out(4)))
! GMS.UC:Lluis Dec.09
           CALL interzdiag (debug, data2, tk, height_field, interz_levels, psfc, ter, tk, qv,            &
             iweg-1, isng-1, ibtg-1, dims_in(4), dims_out(4),                                   &
             num_height_levels, LINLOG, extrapolate, .FALSE., MISSING)
           rcode = nf_put_vara_real (mcid, jvar, start_dims, dims_out, data2)
...
! Write allways equivalent pressures
!        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'PRES') /= 0 ) THEN
           !!! PRES
           jvar = jvar + 1
           varnameIN='PRESSURE'
           ...
           CALL def_var (mcid, jvar, varnameIN, 5, 4, jshape, "XZY", longdescIN, unitsIN, "-",  &
             "XLONG XLAT", output_netCDF4)
           IF (debug) THEN
             write(6,*) 'VAR: PRESSURE idvar:',jvar
             write(6,*) '     DIMS OUT: ',dims_out
           ENDIF
           allocate (data2(dims_out(1),dims_out(2),dims_out(3),dims_out(4)))
! GMS.UC:Lluis Dec.09
           CALL interzdiag (debug, data2, pres_field, height_field, interz_levels, psfc, ter, tk, qv,           &
             iweg-1, isng-1, ibtg-1, dims_in(4), dims_out(4),                                   &
             num_height_levels, LINLOG, extrapolate, .TRUE., MISSING)
           rcode = nf_put_vara_real (mcid, jvar, start_dims, dims_out, data2)
           IF (debug) write(6,*) '     SAMPLE VALUE OUT = ',data2(dims_out(1)/2,dims_out(2)/2,1,&
             1)
           deallocate(data2)
!        ENDIF
        ...
        IF ( INDEX(process,'all') /= 0 .OR. INDEX(process_these_fields,'RH') /= 0 ) THEN
           ...
           allocate (data2(dims_out(1),dims_out(2),dims_out(3),dims_out(4)))
! GMS.UC:Lluis Dec.09
           CALL interzdiag (debug, data2, rh, height_field, interz_levels, psfc, ter, tk, qv,            &
             iweg-1, isng-1, ibtg-1, dims_in(4), dims_out(4),                                   &
             num_height_levels, LINLOG, extrapolate, .FALSE., MISSING)
           WHERE ( rh < 0.0 )
              rh = 0.0
...
!!! Close files on exit
 ...
 END PROGRAM z_interp
!---------------------------------------------------------------------
!---------------------------------------------------------------------
! GMS.UC: Lluis Dec.09
SUBROUTINE extract_from_geogrid(dbg,geoname, variables, Nvar, dx, dy, dz, geogrid_data)
...
section="'extract_from_geogrid'"
IF (dbg) PRINT *,'Subroutine '//TRIM(section)//'... .. .'

PRINT *,"file name: '"//TRIM(geoname)//"'"
rcode = nf_open(geoname, 0, ncgid)

DO ivar=1,Nvar
  PRINT *,'ivar: ',ivar,"var name: '"//TRIM(variables(ivar))//'"'
...
rcode = nf_close ( ncgid )
IF (dbg) PRINT *,'Out from '//TRIM(section)
...
SUBROUTINE variablessum(dbg, ncid, variables, Nvar, dx, dy, dz, dt, datasum)
...
END SUBROUTINE massvertint
 ...
 END SUBROUTINE
!---------------------------------------------------------------------
 SUBROUTINE interz (dbg, data_out, data_in, height_field, interz_levels, psfc, ter, tk, qv, ix, iy, iz, it, &
                     num_height_levels, LINLOG, extrapolate, GEOPT, MISSING)

     INTEGER                                          :: ix, iy, iz, it
     INTEGER                                          :: num_height_levels, LINLOG
     REAL, DIMENSION(ix, iy, num_height_levels, it)   :: data_out
     REAL, DIMENSION(ix, iy, iz, it)                  :: data_in, height_field, tk, qv
     REAL, DIMENSION(ix, iy, it)                      :: psfc
     REAL, DIMENSION(ix, iy)                          :: ter
     REAL, DIMENSION(num_height_levels)               :: interz_levels
     LOGICAL, INTENT(IN)                              :: dbg

     INTEGER                                          :: i, j, itt, k, kk, kin
     REAL, DIMENSION(num_height_levels)               :: data_out1D
     REAL, DIMENSION(iz)                              :: data_in1D, height_field1D
     INTEGER                                          :: extrapolate
     REAL                                             :: MISSING
     REAL, DIMENSION(ix, iy, num_height_levels, it)   :: N
    ...
    IF (dbg) PRINT *,'Subroutine '//TRIM(section)//'... .. .'
     ...
     do itt = 1, it
        ...
        do i = 1, ix
           ...
           height_field1D(:) = height_field(i,j,:,itt)
           CALL int1D (dbg, data_out1D, data_in1D, height_field1D, interz_levels, iz, num_height_levels, LINLOG, MISSING)
 ...
 END SUBROUTINE interz 
!------------------------------------------------------------------------------
! GMS.UC: Lluis
!------------------------------------------------------------------------------
SUBROUTINE spatialfiltering(dbg,datain,grid,ntimes,dx,dy,dz,dt,dataout)
...
!------------------------------------------------------------------------------
 SUBROUTINE mean_sealevelpress (dbg, data_out, pres_field, interp_levels, psfc, ter, tk, qv, ix, iy, iz, it, ito, &
        num_height_levels, LINLOG, extrapolate, MISSING)
! New subroutine to compute mean_sealevel pressure values 

     INTEGER, INTENT(IN)                                           :: ix, iy, iz, it, ito
     INTEGER, INTENT(IN)                                           :: num_height_levels, LINLOG
     ...
     REAL, DIMENSION(num_height_levels), INTENT(IN)                :: interp_levels
     LOGICAL, INTENT(IN)                                           :: dbg
...
!     REAL, DIMENSION(iz)                                          :: data_in1D, pres_field1D
     ...
     kk = 0
     find_kk : do k = 1, num_height_levels
        kk = k
...
! dims_in(4)
 SUBROUTINE interzdiag (dbg, data_out, data_in, height_field, interz_levels, psfc, ter, tk, qv, ix, iy, iz, it, ito, &
                     num_height_levels, LINLOG, extrapolate, GEOPT, MISSING)

     INTEGER, INTENT(IN)                                           :: ix, iy, iz, it, ito
     INTEGER, INTENT(IN)                                           :: num_height_levels, LINLOG
     REAL, DIMENSION(ix, iy, num_height_levels, ito), INTENT(OUT)  :: data_out
     REAL, DIMENSION(ix, iy, iz, ito), INTENT(IN)                  :: data_in
     REAL, DIMENSION(ix, iy, iz, ito), INTENT(IN)                  :: height_field, tk, qv
     REAL, DIMENSION(ix, iy, ito), INTENT(IN)                      :: psfc
     REAL, DIMENSION(ix, iy), INTENT(IN)                           :: ter
     REAL, DIMENSION(num_height_levels), INTENT(IN)                :: interz_levels
     LOGICAL, INTENT(IN)                                           :: dbg
 ...
 END SUBROUTINE interzdiag

!------------------------------------------------------------------------------
!--------------------------------------------------------
 SUBROUTINE int1D(dbg, xxout, xxin, ppin, ppout, npin, npout, LINLOG, MISSING)
...
!! Lluis Feb 11. UC
!                  ELSE
                  IF (pout(np).gt.p(nl) .and. pout(np).lt.p(nl+1)) THEN
		    slope = (x(nl+1)-x(nl)) / (p(nl+1)-p(nl))
		    xout(np) = x(nl) + slope*(pout(np)-p(nl))
                    CYCLE
		  ELSE IF (pout(np) .LT. p(1)) THEN
		    slope = (x(2)-x(1)) / (p(2)-p(1))
		    xout(np) = x(1) + slope*(pout(np)-p(1))  
		    CYCLE
		  ELSE IF (pout(np) .GT. p(nlmax-1)) THEN
		    slope = (x(nlmax)-x(nlmax-1)) / (p(nlmax)-p(nlmax-1))
		    xout(np) = x(nlmax-1) + slope*(pout(np)-p(nlmax-1))
                    CYCLE
...
!"!                      pb = log(pout(np))
		    ...
		    PRINT *,'Height of interpolation', pout(np)
   		    PRINT *,'by a logarithmic method'
		    PRINT *,'can not be !!'
                    PRINT *,'ERROR -- error -- ERROR -- error -- ERROR '
                    STOP
		  END IF
                  IF (pout(np).gt.p(nl) .and. pout(np).lt.p(nl+1)) THEN
                    ...
                    pb = log(pout(np))
		    pc = log(p(nl+1))
		    slope = (x(nl+1)-x(nl))/ (pc-pa)
		    xout(np) = x(nl) + slope*(pb-pa)
		    CYCLE
                  ELSE IF (pout(np) .LT. p(1)) THEN
                    pa = log(p(1))
                    pb = log(pout(np))
...
!"!		    pc = exp(p(2))
		    slope = (x(2)-x(1))/ (pc-pa)
		    xout(np) = x(1) + slope*(pb-pa)
		    CYCLE
		  ELSE IF (pout(np) .GT. p(nlmax-1)) THEN
                    pa = log(p(nlmax-1))
                    pb = log(pout(np))
		    pc = log(p(nlmax))
		    slope = (x(nlmax)-x(nlmax-1))/ (pc-pa)
		    xout(np) = x(nlmax-1) + slope*(pb-pa)
                    CYCLE
		  END IF
              end do
          end do
